...
namespace PokemonGo.RocketAPI.Rpc
{
    public class Inventory : BaseRpc
    {
        ...
        public async Task<int> GetItemAmountByType(ItemData type)
        {
            var pokeballs = await GetItems();
            return pokeballs.FirstOrDefault(i => (ItemId)i.ItemId == type.ItemId)?.Count ?? 0;
        }
        ...
        public async Task<IEnumerable<PokemonData>> GetPokemonToEvolve(IEnumerable<PokemonId> filter = null)
        {
            ...
            myPokemons = myPokemons.Where(p => p.DeployedFortId == "").OrderByDescending(p => p.Cp); //Don't evolve pokemon in gyms
            if (filter != null)
            {
        ...
        public async Task<IEnumerable<PokemonData>> GetDuplicatePokemonToTransfer(bool keepPokemonsThatCanEvolve = false)
        {
            ...
            var pokemonList = myPokemon.Where(p => p.DeployedFortId == "" && p.Favorite == 0).ToList();
            if (keepPokemonsThatCanEvolve)
            {
                ...
                foreach (var pokemon in pokemonsThatCanBeTransfered)
                {
                    ...
                    results.AddRange(pokemonList.Where(x => x.PokemonId == pokemon.Key && PokemonInfo.CalculatePokemonPerfection(x) <= _client.Settings.ivmaxpercent));
                    if(_client.Settings.HoldMaxDoublePokemons > amountToSkip)
                    {
                        amountToSkip = _client.Settings.HoldMaxDoublePokemons;
                    }
            ...
            return pokemonList
                ...
                .Where(x => x.Count() > 1) 
                .SelectMany(p => p.Where(x => x.Favorite == 0 && PokemonInfo.CalculatePokemonPerfection(x) <= _client.Settings.ivmaxpercent)
                .OrderByDescending(x => x.Cp)
                .ThenBy(n => n.StaminaMax)
